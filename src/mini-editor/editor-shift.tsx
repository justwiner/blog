import { CodeTween, CodeConfig, CodeStep } from "../smooth-code";
...
export function useTransition(
  ...
  const { prevNorthFile, prevSouthFile, nextNorthFile, nextSouthFile } =
    ...
    northPanel: {
      ...
      children: (
        <CodeTransition
          ...
          t={t}
          parentHeight={(northStyle.height || northStyle.minHeight) as string}
        />
      ),
    },
    southPanel: inputSouthPanel && {
      ...
      children: (
        <CodeTransition
          ...
          t={t}
          parentHeight={(southStyle?.height || southStyle?.minHeight) as string}
        />
      ),
    },
  };
...
function endingPosition(
  ...
  let { prevNorthFile, prevSouthFile, nextNorthFile, nextSouthFile } =
  ...
  const isTwoToOneSouth =
    !inputSouthPanel && inputNorthPanel.active === prev?.southPanel?.active;
  if (isTwoToOneSouth) {
 ...
 */
function getStepFiles(prev: EditorStep, next: EditorStep, backward: boolean) {
  ...
  const prevSouthFile =
    ps === ns ? psFile : backward ? psFile || nsFile : nsFile || psFile;

  const nextSouthFile =
    ps === ns ? nsFile : backward ? psFile || nsFile : nsFile || psFile;
...
function getPanelTabs(
  ...
  const oldTabs = !nextSnapshot
    ...
    : Object.keys(nextSnapshot)
        .filter(
          (filename) => isPresent(filename, prevSnapshot) || !prevSnapshot
        )
        .map((filename) => {
...
function getPanelSnapshot(parent: HTMLDivElement, step: EditorStep) {
  if (!parent)
    return {
      titleBarHeight: 0,
      northHeight: 0,
  ...
  return {
    ...
    northKey: step.northPanel.active,
    southHeight: southElement?.getBoundingClientRect().height || null,
    southKey: step.southPanel?.active,
  };
}

function getTabsSnapshot(parent: HTMLDivElement, step: EditorStep) {
  if (!parent)
    return {
      northTabs: {},
      southTabs: {},
  ...
  return {
    ...
    southTabs: getTabsDimensions(southTabs, step.southPanel?.active),
  };
}

function getTabsDimensions(tabElements: Element[], active: string | undefined) {
  if (!tabElements[0]) {
    return null;
  }
