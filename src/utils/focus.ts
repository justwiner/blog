...
function mergeToObject<K extends string | number, T>(entries: Record<K, T>[]) {
 ...
 */
function getFocusExtremes(focus: FocusString, lines: any[]): [number, number] {
  if (!focus) {
...
export function getFocusIndexes(focus: FocusString, lines: any[]): number[] {
  if (!focus) {
    // @ts-ignore
    return [...lines.keys()];
  } else {
...
function getFocusSize(focus: Record<LineIndex, true | ColumnIndex[]>) {
...
function fromEntries<K extends string | number | symbol, V>(pairs: [K, V][]) {
...
function makePartAbsolute(part: string, lineNumber: number) {
...
export function mergeFocus(fs1: FocusString, fs2: FocusString) {
  if (!fs1) return fs2 || "";
  if (!fs2) return fs1 || "";
  return `${fs1},${fs2}`;
}
