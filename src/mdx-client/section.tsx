...
export function Section({ children, ...props }: { children: React.ReactNode }) {
  ...
  return (
    <section>
      ...
      >
        <LinkableSection onActivation={setFocus} onReset={resetFocus}>
          {children}
        </LinkableSection>
      </SectionContext.Provider>
    </section>
...
export function LinkableSection({
  ...
  const [activatedId, setActivatedId] = React.useState<any>(undefined);

  const activate = React.useCallback(
    (x: { fileName?: string; focus: string | null; id: string }) => {
      setActivatedId(x.id);
      onActivation(x);
    },
    [onActivation]
