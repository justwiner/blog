...
export function getAnnotationsFromMetastring(options: Record<string, string>) {
...
export function extractJSXAnnotations(
  ...
  while (
    ...
    parent.children[nextIndex].type === "mdxJsxFlowElement" &&
    (parent.children[nextIndex] as JsxNode).name === "CH.Annotation"
  ) {
