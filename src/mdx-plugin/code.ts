...
export async function transformCodeNodes(
  ...
  await visitAsync(tree, "code", async (node: CodeNode, index, parent) => {
    await transformCode({ node, index, parent: parent! }, { theme });
  });
}
...
export function isEditorNode(node: SuperNode) {
  return (
    node.type === "code" ||
    (node.type === "mdxJsxFlowElement" && node.name === "CH.Code")
  );
}
...
export async function mapEditor(
  ...
  const northFiles = await Promise.all(
    northNodes
      ...
      .map((nodeInfo: any) => mapFile(nodeInfo, config))
  );
...
async function mapFile(
  ...
  const metaAnnotations = getAnnotationsFromMetastring(options as any);
  ...
  const file = {
    ...
    code,
    name: options.name || "",
    annotations: [...metaAnnotations, ...commentAnnotations, ...jsxAnnotations],
  };
...
function parseMetastring(metastring: string): FileOptions & { name: string } {
